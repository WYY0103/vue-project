...
2)项目中创建mock文件夹
3)模拟数据 准备JSON数据
4)mock中需要的图片放到public中
5)开始mock数据
6)创建mockServe.js通过mockjs来实现模拟数据
7)mockServe.js在入口文件引入
...
需要在home里面的index.vue进行发请求获取数据  这样才v-for遍历组件进行复用
home组件获取到floor组件的数据，循环产生两个floor，再利用父子组件之间的通信将home的数据传递给floor进行使用
